...
最後更新：2026-06-02 20:xx（**WMOM-20260602-01 done — Knowledge FastAPI router：Alert → RAG auto query（EPIC-M5 M5-6）**）。今日 autonomous daily worker session：preflight baseline 全綠（backend 624 passed / 1 xfailed，無 blocker / 無 regression → 決策樹第 1、2 條不觸發）。依 6/1 handoff 建議接 🔵 autonomous 單 session 項 = **M5-6**（把 M5-1 baseline 檢索層接上 HTTP，為 M5-5 `/field/` 前端鋪路）。**新增** `modules/knowledge/routers/`：`knowledge_router.py`（prefix `/api/knowledge`，2 endpoints：`POST /alert`[AlertEvent→AlertRagResult]、`GET /status`[KnowledgeStatus]）+ `__init__.py`。**DI pattern 對齊既有 4 個 workflow/cost routers**：模組級 `_handler` singleton + `set_alert_handler` setter（`None`→lazy 重建 baseline，docstring 註明不依賴 FarmRegistry）。`alert_handler.py` 加 `retriever`/`strategy` 唯讀 property；`app.py` `include_router`；`KnowledgeStatus` schema 放 `schemas.py`（純 pydantic、維持分層、router 不 re-export）。`top_k=3` 對齊 M5-6「顯示 top-3 chunks」。**升級契約**（MVP_ARCHITECTURE §3.5）：M5-2 ChromaDB / M5-3 真向量檔就緒後只需 `set_alert_handler` 注入新 handler，router/schema 不改。**Verify**：knowledge 54→64 tests（+10：API endpoints + error path）+ backend 全套 **634 passed / 1 xfailed**（624 原 baseline + 10 新，零 regression）；`Any` 0；app 註冊 smoke 2 routes OK；端到端 `POST /alert{alarm_code:21}`→200 top1 變頻器冷卻 SOP `is_baseline=True`。**Code review**：3 must / 5 should / 2 nice → must+should 全採納 + 便宜 nice(NTH9) 採納 / NTH10 不採納（must：兩 endpoint try/except→可讀 500 對齊 cost_router；test docstring「5」→動態取值；lazy init async 安全性 docstring）。frontend 未動（backend-only）vitest 59 baseline 不受影響。issue_stats done 50→51 / total 64→65；M5 progress 15→25。下一步候選：M5-5 `/field/` mobile UI（呼叫 `POST /api/knowledge/alert` 顯示 top-3，🔵 PMF 關鍵）/ M5-2 ChromaDB（🔵 需 chromadb 依賴）/ M5-3 RAG_Ultimate 真向量檔（🟡）。詳細 handoff 在 work-logs/2026-06/2026-06-02-knowledge-api-router.md。
...
| M5-6 | **Alert → RAG auto query**：警報事件觸發即 retrieve，前端顯示 top-3 chunks | 🔵 ✅ | **done（WMOM-20260602-01）**：`POST /api/knowledge/alert` + `GET /status` |
...
### WMOM-20260602-01 — Knowledge FastAPI router：Alert → RAG auto query（EPIC-M5 M5-6）

- **Status**: done（2026-06-02 完成）
- **Milestone**: M5
- **Priority**: high（串前端關鍵，為 M5-5 `/field/` 鋪路）
- **Owner**: Claude (autonomous daily worker, session 2026-06-02)
- **Completion summary**:
  - ✅ **`modules/knowledge/routers/knowledge_router.py`**（新）— prefix `/api/knowledge`，2 endpoints：
    - `POST /alert`：body `AlertEvent` → `AlertRagResult`（告警碼對應 top-k SOP chunks + 可解釋 match_reason + `is_baseline`）；
      naive timestamp 自動 422（CLAUDE.md §B UTC-aware）；檢索層拋錯 → 可讀 500（對齊 cost_router）
    - `GET /status`：→ `KnowledgeStatus`（retriever / is_baseline / strategy_name，前端 baseline badge 用）
  - ✅ **DI pattern**：模組級 `_handler` singleton + `set_alert_handler` setter（`None`→lazy 重建 baseline），對齊既有 4 個 workflow/cost routers
  - ✅ `alert_handler.py` 加 `retriever`/`strategy` 唯讀 property；`app.py` `include_router`；`KnowledgeStatus` 放 `schemas.py`（維持分層）
  - ✅ **升級契約**（MVP_ARCHITECTURE §3.5）：M5-2 ChromaDB / M5-3 真向量檔就緒後只需 `set_alert_handler` 注入新 handler，router/schema 不改
  - ✅ **64 tests**（knowledge 54→64，+10 API endpoints + error path）；backend 全套 634 passed / 1 xfailed 零 regression；`Any` 0
  - ✅ **Code review**（code-reviewer subagent）：3 must / 5 should / 2 nice → must+should 全採納 + 便宜 nice(NTH9) 採納 / NTH10 不採納
- **不在本 issue 範圍**（後續 epic 子目標）：
  - M5-5 `/field/` mobile-first frontend（呼叫 `POST /api/knowledge/alert` 顯示 top-3）🔵（下一步建議候選，PMF 關鍵）
  - M5-2 ChromaDB 整合 🔵（需 chromadb 依賴）/ M5-3·M5-4 接 RAG_Ultimate Phase 3 🟡
  - manual 全文搜尋 endpoint（非告警驅動）— 等 M5-5 前端 UX 確定再開 issue
- **Reference**: [`work-logs/2026-06/2026-06-02-knowledge-api-router.md`](work-logs/2026-06/2026-06-02-knowledge-api-router.md)
