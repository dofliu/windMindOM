...
最後更新：2026-06-07（**WMOM-20260607-03 done — EventComparisonView component render 測試（EPIC-M5 測試覆蓋擴大）**）。autonomous worker session（2026-06-07 第三輪）：preflight 全綠（backend 638 / frontend 719、飛輪健康：上輪 TrendChartPanel PR #97 已 auto-merge 進 main，baseline 自 692 推進到 719）。stack-aware：open PR 全為飛輪上線前 stale draft（#30–#68 共 22 筆），無進行中 WIP。決策樹 #1/#2/#3 皆無 → 落 #4 乾淨 autonomous 工作。接上輪 handoff「render 測試剩餘 untested 元件」清單下一支——`components/EventComparisonView.tsx`（332 行，多風機事件比較分析面板）。**新增** `components/__tests__/EventComparisonView.test.tsx`（**32 tests**——初版 28 + review 採納補 4，純測試零 production 變更）。**Mock 策略**：`useTheme` 用真實 ThemeProvider + ui 元件；`global.fetch` 以 `vi.fn` 路由 `/api/maintenance/events/compare` 回 `{timeline,summary,farm_events}`（未預期 URL reject）；`window.open` stub 成 `vi.fn` 驗匯出 CSV URL。turbine fixture 5 台（WT001-005，元件預設選前 4 台）。覆蓋 殼層標題（zh/en/預設 lang）/ 風機選擇器（5 顆 toggle·預設前 4 pressed·toggle 加入取消·全選·清除+摘要隱藏·en All/None）/ filters 匯出（事件類型 Select zh/en·匯出 CSV→window.open `/api/export/events?format=csv`）/ fetch 接線（mount compare 帶 turbine_ids+limit=500·選類型/變更日期觸發新 fetch·清除後 selectedIds 空不再 fetch）/ 狀態列（N 事件·M 台·載入中…·清除後 0 台）/ 單機摘要（total+by_type pills·缺台 fallback 0）/ 全場事件（有資料顯示·空陣列不渲染）/ 時間線（標題+severity pill·detail 行內文字·_turbine_id 優先+— fallback·空狀態 zh/en）/ 容錯（fetch reject 不崩潰·空 turbines 無按鈕無 fetch）。**眉角**：WT### 同現於選擇器/摘要/時間線→改用 distinctive total/pill/title 斷言避多元素命中；turbine_ids 逗號 URLSearchParams encode 為 `%2C`→regex 容忍 `,`/`%2C`。code-reviewer 回 0 must-fix+4 should-fix+6 nice，**採納 8 / 婉拒 2**（should: waitFor 改等 fetch 產物·getAllByText toHaveLength(3)·補 All option·補完整 timeline fixture；nice: loading 態·detail·_turbine_id fallback·datetime refetch；婉拒 severity 空佔位、farmEvents slice30 噪音），28→32 tests。**Verify**：`tsc` 0 error + `vitest` **751 passed**（719 baseline + 32 新，零 regression）+ `vite build` ✓；backend 未動 638 / 1 xfailed。issue_stats done 78→79 / total 92→93。**下一步**：render 測試剩餘 untested 元件（MaintenanceHub 439 / FarmSelector 532 / FaultInjectionPanel 555 / ui primitives）；非 render M5-2 ChromaDB（🟡）/ M5-5 `/field/` mobile Part B-2（🟡）/ stale PR triage（#30–#68 共 22 筆）。詳細 handoff 在 work-logs/2026-06/2026-06-07-eventcomparisonview-render-tests.md。

> 📁 更早一筆 changelog（WMOM-20260607-02 TrendChartPanel）詳見 work-logs/2026-06/2026-06-07-trendchartpanel-render-tests.md；各 issue 詳細紀錄仍保留於下方 `### WMOM-*` 區段，更早 session 的 changelog blurb 已封存到 `docs/legacy/issues_changelog_archive.md`。
