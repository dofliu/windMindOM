...
最後更新：2026-06-06（**WMOM-20260606-07 done — MonthlyReportPanel component render 測試（EPIC-M5 測試覆蓋擴大）**）。autonomous worker session（同日第七輪）：preflight 全綠（backend 638 / frontend 595、飛輪健康：上輪 DispatchModal PR #93 已 auto-merge 進 main，baseline 自 574 推進到 595）。stack-aware：open PR 全為飛輪上線前 stale draft（#30–#68 共 22 筆），無進行中 WIP。決策樹 #1/#2/#3 皆無 → 落 #4 乾淨 autonomous 工作。挑 reporting 子面板中**商業關聯度最高**的 `components/reporting/MonthlyReportPanel.tsx`（357 行）——A9 月報生成面板，**直接餵 M6-6「第一份自動月報交業主」** deliverable，先前頁面層只有 `ReportsPage`（mock 掉本面板）+ `formatters` 單元測試覆蓋，本面板本體零 render 覆蓋。**新增** `components/reporting/__tests__/MonthlyReportPanel.test.tsx`（**33 tests**，純測試零 production 變更）。依賴僅 `useTheme` → ThemeProvider 包裹；純 props-driven，唯一 async 副作用 `onDownloadPdf` 以受控 Promise 驗下載中態+失敗錯誤卡；`Btn`/`Card`/`Field`/`Select`/`Stat` 用真實 ui 元件不 mock。**時間眉角**：預設選上個月依賴 `new Date()`，初版誤用 `vi.useFakeTimers()` 卡死 `waitFor` polling（5 async timeout）→ 改不 fake、用與元件相同邏輯動態算期望年月。覆蓋 filter bar（年月 Select+Generate·farmId 空 disabled·預設上月·年份 cur-4…cur+1）/ 生成接線（onGenerate 帶當前選擇·loading 中態）/ empty state 四態 / KPI 5 卡格式化（€1.23M·73.2%·8·4.50 h·96.1%）+ header 風場名/年月 zero-pad/產出時間 / 4 大類成本表+合計列+`within` row scope+非數字退 dash「—」/ PDF 下載（無 data 無鈕·點下載帶 year+month·下載中態恢復·失敗錯誤卡）/ 雙 error 獨立顯示+generate 清舊 download error / HTML iframe（srcDoc+`sandbox=""`）/ 語系 / NaN 降級。code-reviewer 回 3 must-fix+4 should-fix+3 nice-to-have，**採納 8**（must#2 `.closest('tr')!` 改 null check、must#3 download 中間態 `waitFor` 內重查避 stale ref、should#4 `getByText('8')` 改 `within` scope、should#5 移除無用 `farm_name` override、should#6 同步呼叫去 `waitFor`、should#7 補 getMonth 註解、nice#8 NaN edge case、nice#9 en 欄表頭斷言），**婉拒 2**（must#1 regex getByText 多元素為**誤判**——`getByText` 用 `getNodeText` 只取直接 text 子節點非 `textContent`，suite 實跑全綠；nice#10 unhandled rejection 未觀察到）。**Verify**：`tsc` 0 error + `vitest` **628 passed**（595 baseline + 33 新，零 regression）+ `vite build` ✓；backend 未動 638 / 1 xfailed。issue_stats done 74→75 / total 88→89。**下一步**：render 測試剩餘 untested 元件（AnnualBudgetPanel 姊妹面板 / UserSwitcher / EventComparisonView / MaintenanceHub / FarmSelector / FaultInjectionPanel / TrendChartPanel / ui primitives）；非 render 方向 M5-2 ChromaDB（需劉老師拍板依賴+向量檔來源）/ M5-5 `/field/` mobile Part B-2（需劉老師拍板現場工程師身分/完工流程）/ stale PR triage（#30–#68 共 22 筆）。詳細 handoff 在 work-logs/2026-06/2026-06-06-monthlyreportpanel-render-tests.md。

> 📁 更早一筆 changelog（WMOM-20260606-06 DispatchModal）詳見 work-logs/2026-06/2026-06-06-dispatchmodal-render-tests.md（更早的 WMOM-20260606-05 TurbineDetail 已搬 `docs/legacy/issues_changelog_archive.md`）。
