...
passed 不變；frontend 1246→1248 passed（+2 新測）、tsc 0、build OK。
...
## 📌 2026-07-18 session 新增 issue（模擬運作模式 + 機組資料修正）
...
- **WMOM-20260718-03** — 🟡 #2 scenario-setup 流程（雙 PR）。
  ...
  - **前端情境頁（#126 merged）✅**：新增「情境」nav 獨立頁 `ScenarioPage`（選風況 profile + 時長 +
    解析度 + 故障排程增刪 → POST wind + generate-bulk → 結果卡 + 跳歷史探索）；`authFetch` enforce-ready；
...
- **WMOM-20260718-04** — 🔵 #3 狀態可見性。
  - **主 PR（#128 merged）✅**：turbine 詳情在功率≈0 時顯示「為何不發電」chip（`noPowerReason`）+
    App header 當前風場 context strip（風場名 + 台數 + 資料來源）。+12 tests。
  - **code review 修正（本 PR）**：🔴 `dataSourceLabel` 修 `MODBUS_TCP` 誤標成「模擬」（真實資料源）；
...
- **WMOM-20260719-02** — ✅ **#4 情境保存/調閱（雙 PR，done）**。根因＝`generate-bulk` 把資料寫進
  當前 active session、無情境識別 → 融進歷史。情境建模成 `config_json.kind=="scenario"` 的專屬
  ended session。
  - **後端（#138 merged）✅**：`query_history` 加 `session_id` 隔離；storage 加 `list/get/delete_scenario`
    + `update_session_config` + `SCENARIO_KIND` + 4 表 session_id 索引；`generate-bulk` 帶 `name` →
    開情境 session 寫入/回填/end（失敗 try/except 收尾）、回傳 `scenario_id`；新 `/api/scenarios`
    router（list/get/history/delete，delete=ADMIN）。+11 tests（7 storage + 4 endpoint），端到端 verify。
...
- **Description**（逐檔盤點，行號為稽核當下，`FaultInjectionPanel` 已修復不再是當前行號）：
  - [x] ~~`FaultInjectionPanel.tsx:92,96,106,114,131,148`~~ — ✅ 2026-09-24 完成
    ...
    mutation-verified。
  - [x] ~~`FarmSelector.tsx:71,85,287`~~ — ✅ 2026-09-24 完成（WMOM-20260924-02）。GET
    `/api/farms`、POST `/api/farms/{id}/activate`、POST `/api/farms`（**建立/切換 farm 皆
  ...
  - [x] ~~`HistoryPage.tsx:145,158`~~ — ✅ 2026-09-24 完成（WMOM-20260924-06）。GET
...
### WMOM-20260924-02 — `FarmSelector.tsx` authFetch 補齊（WMOM-20260923-10 sub-task）

- **Status**: done
- **Milestone**: M6 auth cutover（`WMOM-20260716-05i`）前置阻塞——`WMOM-20260923-10` 稽核清單第二項
- **Priority**: high（sidebar 底部風場切換器 + 建立風場 modal 共 3 處端點，切換/建立皆
  `SUPERVISOR`/`ADMIN`-only 寫入，enforce 開啟後裸 fetch 會使 farm 管理功能整面失效）
- **Estimate**: 40 min（實際）
- **Source**: `WMOM-20260923-10`（前端 authFetch 稽核，2026-09-23）稽核清單第二項，本次 autonomous
  session 接手
- **Description**:
  `frontend/components/FarmSelector.tsx`（sidebar 底部風場切換器）3 處 fetch 呼叫（GET
  `/api/farms`、POST `/api/farms/{farmId}/activate`、POST `/api/farms`）全部裸 `fetch`，未帶
  `Authorization` header。後端列表端點掛 `require_authenticated()`，切換/建立兩個寫入端點皆掛
  `require_role(Role.SUPERVISOR)`。
- **Deliverable**：
  - `FarmSelector.tsx`：`import { authFetch } from '../services/authClient'`，3 處
    `fetch(` → `authFetch(`（僅替換呼叫方式，method/headers/body 參數完全不動）。
  - `FarmSelector.test.tsx`：新增「authFetch 稽核」describe block，4 測——已登入時 mount GET +
    切換 POST activate + 建立 POST /api/farms 皆帶 `Authorization: Bearer <token>`；未登入時
    三處皆不帶（驗證 `WMOM_AUTH_ENFORCE=false` 過渡期行為不變）。
- **Verify**：
  - Mutation-verified：暫時 sed 把 3 處 `authFetch(` 改回 `fetch(`，重跑此檔測試 → 3 個「已登入」
    新測試如預期 fail（mount + 切換 + 建立），「未登入」測試維持 pass（符合預期——該測試本就只
    驗證無 token 時三種寫法行為一致，不區分 fetch vs authFetch），已還原確認 `git diff --stat`
    乾淨、3 處 `authFetch` 呼叫皆在。
  - backend 未動（純 frontend 改動）1103 passed 不變；frontend `npx tsc --noEmit` 0 error、
    `npx vitest run` 1232→1236 passed（58 files 不變，+4 新測）、`npx vite build` OK。
- **Review**: code-reviewer subagent review：0 must-fix、0 should-fix、2 nice-to-have（「未登入」
  測試對 fetch/authFetch 不敏感、`callsMatching`/`authHeaderOf` 與姊妹檔 `calls()` 邏輯些微重複，
...
### WMOM-20260924-06 — `HistoryPage.tsx` authFetch 補齊（WMOM-20260923-10 sub-task）

- **Status**: done
- **Milestone**: M6 auth cutover（`WMOM-20260716-05i`）前置阻塞——`WMOM-20260923-10` 稽核清單第六項
- **Priority**: medium（純讀取端點，`WMOM_AUTH_ENFORCE=false` 過渡期不影響功能，但 cutover
  前必須清空）
- **Estimate**: 25 min（實際）
- **Source**: `WMOM-20260923-10`（前端 authFetch 稽核，2026-09-23）稽核清單第六項，本次
  autonomous session 接手
- **Description**:
  `frontend/components/HistoryPage.tsx:145,158` 2 處裸 `fetch`：
  1. mount 時（`useEffect([lang])`）GET `${API_BASE}/api/i18n/tags?lang=...`（標籤 i18n 對映）
  2. mount + 篩選條件變更時 GET `${API_BASE}/api/turbines/{id}/history?...`（帶
     `{ signal: ctrl.signal }`，`AbortController` 中止語意）。
  交叉核對後端兩端點皆掛 `require_authenticated()`（任何登入者可讀）。（同檔案
  `exportCsv()` 的兩處 `window.open(...)` 屬瀏覽器導覽下載、非 `fetch` 呼叫，無法附帶
  header，不在本次範圍內，行為與既有 `FarmOverview.tsx`/`EventComparisonView.tsx` 匯出模式
  一致。）
- **Deliverable**：
  - `HistoryPage.tsx`：`import { authFetch } from '../services/authClient'`，2 處
    `fetch(` → `authFetch(`（僅替換呼叫方式，`.then/.catch/.finally` 鏈與 `AbortController`
    完全不動）。
  - `HistoryPage.test.tsx`：新增「authFetch 稽核」describe block，2 測——已登入時兩條 mount
    GET 皆帶 `Authorization` header、未登入時皆不帶 header（過渡期行為不變對照組）。
- **Verify**：
  - Mutation-verified：暫時用備份還原法（`/tmp/HistoryPage.tsx.bak`，非 `git checkout`）把
    兩處 `authFetch(...)` 改回 `fetch(...)`，重跑此檔測試 → 「已登入」新測試如預期 fail
    （`expected undefined to be 'Bearer ...'`）、「未登入」對照組維持 pass（本質斷言
    undefined，非測試盲區），已還原確認修復仍在、全檔 23 測全數 pass。
  - backend 未動（本次純前端變更）：`1103 passed, 7 skipped, 1 xfailed`，與既有基準一致；
    frontend `npx tsc --noEmit` 0 error、`npx vitest run` 1248→1250 passed（58 files 不變，
    +2 新測）、`npx vite build` OK。
- **Review**: code-reviewer subagent review：Approve，0 must-fix、0 should-fix、2
...
- **Completion summary**：新增 `frontend/components/__tests__/WorkOrderDetailModal.
  test.tsx`（新檔，18 tests），**未修改元件本體任何一行**。涵蓋標題/詳情/技師名稱
  ...
  tsc 0、1443→**1461 passed**（68→69 files，+18，零 regression）、build OK。
- **code-reviewer subagent review：Approve，0 must-fix，2 should-fix + 3
  nice-to-have，2 個 should-fix 全數已修復**：①reviewer 用獨立 mutation 實驗發現
  「At least one photo is required…」提示文字實際只受 `!isCompleted` 控制、與照片
  數量無關，原測試只在無照片情境驗證易誤導成「依數量 gating」——補 1 則測試鎖住「有
  照片時提示仍顯示」的真實行為；②「無照片時點擊 Complete 不觸發 onComplete」測試旁補
  程式碼註解，明講此測試只鎖住 DOM 層 `disabled` 屬性、內部邏輯層 guard 因 jsdom 對
  disabled button 不派發 click handler 而無法被此測試獨立驗證。3 個 nice-to-have
  （`technicianId: null` 型別分支、多檔上傳等邊界情境、Save 的反向斷言）未採納，留待
  未來第二輪測試。
- **誠實揭露**：`handleComplete` 內部 `if (photos.length > 0)` 邏輯層 guard 目前無法
  獨立於 DOM 層 `disabled` 屬性被測到——jsdom 對 `disabled` 的 `<button>` 不會派發
  `click` 事件的 React handler（瀏覽器原生行為），mutation-verify 與 reviewer 獨立
  實驗皆證實拿掉內部 guard（只留 `disabled`）測試仍全過。此邏輯層 guard 是防禦性重複
  （defense-in-depth），非本次新增缺口，已在測試檔對應 `it()` 名稱與程式碼註解誠實
  標註「disabled 阻擋」而非宣稱鎖住內部邏輯。
