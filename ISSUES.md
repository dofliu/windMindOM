...
最後更新：2026-06-06（**WMOM-20260606-04 done — MaterialRequestDetailModal component render 測試（EPIC-M5 測試覆蓋擴大）**）。autonomous worker session（同日第四輪）：preflight 全綠（backend 638 / frontend 481、飛輪健康：上輪 WorkOrderDetailModal PR #90 已 auto-merge 進 main）。stack-aware：open PR 全為飛輪上線前 stale draft（#30–#68 共 22 筆），無進行中 WIP。決策樹 #1/#2/#3 皆無 → 落 #4 乾淨 autonomous 工作。依前份 handoff 點名「下一個 detail modal」挑 **MaterialRequestDetailModal**（`components/workflow/MaterialRequestDetailModal.tsx`，902 行，WorkOrderDetailModal 姊妹元件）：`/admin/workflow` 領料單詳情 + 狀態機 transition modal，依 `status` 顯示 footer action（submit/dispatch/receive/close/cancel/return），點按開 inline form 收必填欄位 → 呼 callback → 成功 patch 本地 MR + 收 form / 失敗回填錯誤卡；receive 自動以 estimated_qty 預填 + 提交前驗證非負整數；return 組裝 CreateMaterialReturnPayload。**新增** `components/workflow/__tests__/MaterialRequestDetailModal.test.tsx`（**44 tests**，純測試零 production 變更：`makeItem`/`makeMR` 工廠結構式滿足型別不用 `as`；`resolveWith(base,patch)` 產回傳帶 status 變化 mock；ThemeProvider+UserProvider 雙層包裹、`beforeEach` 清 localStorage 讓 fallback DEFAULT_USER 穩定、actor 斷言用 `DEFAULT_USER.id`；覆蓋殼層·timeline grid Asia/Taipei 格式 vs dash·items table 缺漏 fallback·actual_qty·cancel/reject 原因卡·lang=en / status-driven 9 態含 closed·cancelled·rejected 全空 / submit·dispatch·close 確認流程+成功狀態更新+footer 鎖定+失敗錯誤卡+submitting 中態+form 取消不呼 / cancel 空·純空白 disabled+trim / receive 預填 estimated_qty·編輯·負數·空 NaN·**0 合法**·多 item record / return 未選 disabled·payload trim·數量<1 disabled·空 note→null·失敗錯誤卡 / 關閉路徑 backdrop·stopPropagation·footer Close·header ✕）。**眉角**：與 WorkOrderDetailModal 不同，footer 開啟鈕與 form 提交鈕 aria-label 互不相同 → 多數可直接 getByRole，**不需** formSubmit scope；唯一重複「關閉」（header ✕ + footer Close）→ `footerClose()` 取最後一個；`…${slice}` fallback 因「…」與 slice 分屬不同 text node → 斷言改 substring regex。**Verify**：`tsc` 0 error + `vitest` **525 passed**（481 baseline + 44 新，零 regression）+ `vite build` ✓；backend 未動 638 / 1 xfailed 不受影響。issue_stats done 71→72 / total 85→86。**下一步**：`TurbineDetail`（1056，monitoring 大元件，需 mock 即時資料）；非 render 方向 M5-2 ChromaDB（需劉老師拍板依賴+向量檔來源）/ stale PR triage（#30–#68 共 22 筆）。詳細 handoff 在 work-logs/2026-06/2026-06-06-materialrequestdetailmodal-render-tests.md。

> 📁 更早一筆 changelog（WMOM-20260606-03 WorkOrderDetailModal）詳見 work-logs/2026-06/2026-06-06-workorderdetailmodal-render-tests.md。

<details><summary>WMOM-20260606-03 — WorkOrderDetailModal component render 測試（已封存，點開看）</summary>

autonomous worker session（同日第三輪）：preflight 全綠（backend 638 / frontend 439、飛輪健康：上輪 CreateMaterialRequestWizard PR #89 已 auto-merge 進 main）。stack-aware：open PR 全為飛輪上線前 stale draft（#30–#68），無進行中 WIP。決策樹 #1/#2/#3 皆無 → 落 #4 乾淨 autonomous 工作。依前份 handoff 點名「detail modal 系列」挑 **WorkOrderDetailModal**（`components/workflow/WorkOrderDetailModal.tsx`，933 行）：`/admin/workflow` 工單詳情 + 狀態機 transition modal，依 `status` 顯示 7 個 action 按鈕，點按開 inline form 收必填欄位 → 呼 callback → 成功 patch 本地 WO + 收 form / 失敗回填錯誤卡，含 offshore start_work 強制 weather_window 分支。**新增** `components/workflow/__tests__/WorkOrderDetailModal.test.tsx`（**42 tests**：`makeWO` 工廠結構式滿足 `WorkOrderResponse` 不用 `as`；`resolveWith(base,patch)` 產回傳帶 status 變化的 mock；ThemeProvider+UserProvider 雙層包裹、`beforeEach` 清 localStorage 讓 fallback DEFAULT_USER 穩定；覆蓋殼層·detail grid 有值 vs —·progress_notes 空 vs 有值計數+內容·完工摘要 closed·cancel/reject/reopen 原因卡·awaiting_signoff 待簽核提示·lang=en 含 action 按鈕 / status-driven 按鈕可見性 7 態含 cancelled 全空 / dispatch form 空·純空白 assignee→fallback currentUser.id·已有 assignee 留空→既有·輸入 trim·成功本地狀態更新·失敗錯誤卡不收合·失敗後 Cancel 清錯誤·submitting「派工中…」disabled / start_work onshore→(id,false)·offshore 空 weather window disabled·填值→(id,true,wwid) trim / progress 空·純空白 disabled·填→onUpdateProgress trim / finish 空·純空白驗證錯誤·0 工時合法·填→完整 payload trim null·followup_needed→額外欄位+followup_note trim / reject·cancel·reopen 各 gating+callback·非 dispatch error path / 關閉路徑 backdrop·stopPropagation·footer Close）。另含一行 **production 修正**（review must-fix）：finish 驗證 `!actualHours`→`!actualHours.trim()`，避免「只敲空白」被 `Number('   ')=0` 當成 0 工時靜默送出（與全 codebase `.trim()` 慣例一致）。**眉角**：footer 開啟鈕與 form 提交鈕 aria-label 同字串（駁回/開始作業/重開/取消工單）→ 抽 `formSubmit(cancelName,submitName)` helper 以 form 內唯一 Cancel/返回 鈕 parentElement scope；submitting 受控 Promise 斷言中間態+收尾 `act(resolve)` 收 form 避免 act 警告。兩輪 code-reviewer review（採納 3 must-fix + 多 should/nice，情形見 work-log）。**Verify**：`tsc` 0 error + `vitest` **481 passed**（439 baseline + 42 新，零 regression）+ `vite build` ✓；backend 未動 638 / 1 xfailed 不受影響。issue_stats done 70→71 / total 84→85。**下一步**：`MaterialRequestDetailModal`（902，另一 detail modal）/ `TurbineDetail`（1056）；非 render 方向 M5-2 ChromaDB（需劉老師拍板依賴+向量檔來源）/ stale PR triage（#30–#68 共 22 筆）。詳細 handoff 在 work-logs/2026-06/2026-06-06-workorderdetailmodal-render-tests.md。
