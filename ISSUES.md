...
最後更新：2026-06-07（**WMOM-20260607-02 done — TrendChartPanel component render 測試（EPIC-M5 測試覆蓋擴大）**）。autonomous worker session（2026-06-07 第二輪）：preflight 全綠（backend 638 / frontend 692、飛輪健康：上輪 UserSwitcher PR #96 已 auto-merge 進 main，baseline 自 665 推進到 692）。stack-aware：open PR 全為飛輪上線前 stale draft（#30–#68 共 22 筆），無進行中 WIP。決策樹 #1/#2/#3 皆無 → 落 #4 乾淨 autonomous 工作。接上輪 handoff「render 測試剩餘 untested 元件」清單首位——`components/TrendChartPanel.tsx`（225 行，風機詳情頁即時趨勢圖面板）。**新增** `components/__tests__/TrendChartPanel.test.tsx`（**27 tests**——初版 25 + review 採納補 2）+ `TrendChartPanel.tsx` footer 加 `data-testid`（穩定選取微 production 變更）。**Mock 策略**：`useTheme` 用真實 ThemeProvider；`recharts` 全 stub 成 marker div（與 CostPage.test 一致，避 jsdom 0 寬度噪音）；`global.fetch` 以 `vi.fn` 依 URL 路由（`/api/i18n/tags` 回 tag label、`/trend` 回 `{data}`，未預期 URL reject），輪詢/cleanup 專屬測試用 fake timer + `advanceTimersByTimeAsync`。覆蓋 標題殼層（zh/en/預設 lang）/ preset 按鈕（7 顆 zh+en·預設 power aria-pressed·高亮轉移）/ 自訂 tag（placeholder+套用·切 tags 清高亮·trim·空白純逗號 guard）/ fetch 接線（mount i18n 帶 lang·trend 帶 turbineId+power tags+limit=120·點 preset 觸發新 tags fetch）/ i18n label 解析 footer（有 label·無 label raw·部分混合·zh/en 前綴）/ 輪詢 cleanup（每 2s·unmount clearInterval）/ 容錯（fetch reject 不崩潰·trend 缺 data 不崩潰）。**眉角**：footer 列以「葉節點 div + textContent 前綴」matcher 精確抓避多元素命中；trim 驗證同時斷言 footer 文字與 trend fetch URL（`tags=FOO,BAR`）。code-reviewer 回 4 must-fix+5 should-fix+3 nice，**採納 8 / 婉拒 2**（renderPanel lang 預設·unmount 包 act·footer data-testid·空 label waitFor·aria-pressed 改 getByRole pressed·trim slice+waitFor·容錯走 installFetch 封裝·guard 補 fetch 次數·turbineId/lang rerender；婉拒 recharts mock 補未用元件、tag 命名另案），25→27 tests。**Verify**：`tsc` 0 error + `vitest` **719 passed**（692 baseline + 27 新，零 regression）+ `vite build` ✓；backend 未動 638 / 1 xfailed。issue_stats done 77→78 / total 91→92。**下一步**：render 測試剩餘 untested 元件（EventComparisonView 332 / MaintenanceHub 439 / FarmSelector 532 / FaultInjectionPanel 555 / ui primitives）；非 render M5-2 ChromaDB（🟡）/ M5-5 `/field/` mobile Part B-2（🟡）/ stale PR triage（#30–#68 共 22 筆）。詳細 handoff 在 work-logs/2026-06/2026-06-07-trendchartpanel-render-tests.md。autonomous worker session（2026-06-07 第一輪）：preflight 全綠（backend 638 / frontend 665、飛輪健康：上輪 AnnualBudgetPanel PR #95 已 auto-merge 進 main，baseline 自 628 推進到 665）。stack-aware：open PR 全為飛輪上線前 stale draft（#30–#68 共 22 筆），無進行中 WIP。決策樹 #1/#2/#3 皆無 → 落 #4 乾淨 autonomous 工作。接上輪 handoff「render 測試剩餘 untested 元件」清單首位（最小一支）——`components/UserSwitcher.tsx`（231 行，sidebar mock-login 切換器，WMOM-20260510-01 Part B 雙 persona demo 關鍵）。**新增** `components/__tests__/UserSwitcher.test.tsx`（**27 tests**——初版 24 + review 採納補 3，純測試零 production 變更）。**Mock 策略**：元件 props 僅 `lang`、資料全來自 `useCurrentUser` context → mock 掉該 hook（注入 currentUser/availableUsers 含一名停用者驗 `is_active` filter + spy `setCurrentUser`），`useTheme` 用真實 ThemeProvider；provider 本體（localStorage 持久化/fixtures）已由既有 `services/__tests__/mockUsers.test.ts` 20 tests 獨立覆蓋。覆蓋 殼層（trigger aria-haspopup/expanded·名稱+role·多角 join·treasury 單角）/ 展開收合 / 選項清單（is_active filter·aria-selected·使用中·dev tag·role join+email·email 空·空陣列）/ 選取接線（click·Enter·Space·自選自己·Tab 不觸發）/ 關閉路徑（click-outside·dropdown 內 mousedown 不關·Escape）/ 語系 en。**眉角**：多元素命中一律 `within(trigger/listbox)` 或 `closest('[role=option]')` scope；「使用中」用 `toHaveLength(1)` 守唯一。code-reviewer 回 1 must-fix+4 should-fix+2 nice-to-have，**採納 5**（should#2 restoreAllMocks 一致·should#3 Carol treasury fixture+test·should#5 自選自己 test·nice#6 email-empty 改 BOB id 去 isActive 混淆·nice#7 空陣列 edge case），**婉拒 2**（**must#1 誤判**——指 `getByText('Owner (dev)')` 必拋錯，但 RTL 預設以 `getNodeText` 只比對直接 text-node、不含巢狀 dev span，實證 24 tests 全綠含被點名 3 條；should#4 移除冗餘 setCtx 為低價值 churn）。**Verify**：`tsc` 0 error + `vitest` **692 passed**（665 baseline + 27 新，零 regression）+ `vite build` ✓；backend 未動 638 / 1 xfailed。issue_stats done 76→77 / total 90→91。**下一步**：render 測試剩餘 untested 元件（TrendChartPanel 225 recharts / EventComparisonView 332 / MaintenanceHub 439 / FarmSelector 532 / FaultInjectionPanel 555 / ui primitives）；非 render M5-2 ChromaDB（🟡 需劉老師拍板依賴+向量檔來源）/ M5-5 `/field/` mobile Part B-2（🟡）/ stale PR triage（#30–#68 共 22 筆）。詳細 handoff 在 work-logs/2026-06/2026-06-07-userswitcher-render-tests.md。

> 📁 更早一筆 changelog（WMOM-20260607-01 UserSwitcher）詳見 work-logs/2026-06/2026-06-07-userswitcher-render-tests.md；各 issue 詳細紀錄仍保留於下方 `### WMOM-*` 區段，更早 session 的 changelog blurb 已封存到 `docs/legacy/issues_changelog_archive.md`。
