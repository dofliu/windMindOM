...
function installFetch(
  ...
  opts: { statusOk?: boolean; selectOk?: boolean } = {},
) {
  ...
  fetchMock = vi.fn((url: string) => {
    ...
    if (url.includes('/api/source/select')) return jsonRes({ status: 'ok' }, selectOk, selectOk ? 200 : 401);
...
describe('useSourceGate', () => {
  ...
  it('selectMode 成功 → POST select、sourceActive=true、回 true', async () => {
    const { result } = renderHook(() => useSourceGate(true));
    await waitFor(() => expect(result.current.sourceActive).toBe(false));
    let ret = false;
    await act(async () => {
  ...
  it('selectMode 失敗（非 ok）→ 回 false、不誤設 active', async () => {
    installFetch({ active: false }, { selectOk: false });
    const { result } = renderHook(() => useSourceGate(true));
    await waitFor(() => expect(result.current.sourceActive).toBe(false));
    let ret = true;
    await act(async () => {
  ...
  it('登出（true→false）不重查 status；重新登入才重查', async () => {
    const { result, rerender } = renderHook(({ auth }) => useSourceGate(auth), {
