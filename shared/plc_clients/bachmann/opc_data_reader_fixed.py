...
"""
OPC 資料持續擷取程式 - 改進版
修正了 SyncRead 可能無回應的問題，增加了逾時處理和錯誤診斷功能。
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Bachmann OPC 資料擷取與診斷工具 - 改進版"
    )
    parser.add_argument(
        "--diag-only",
        action="store_true",
    ...
    parser.add_argument(
        "--use-async",
        action="store_true",
        help="使用非同步讀取模式（可能可以避免卡住，但需要更多測試）。",
    )
    parser.add_argument(
        "--read-batch-size",
        type=int,
        default=5,
        help="每次讀取的標籤批次大小（較小的批次可能更穩定）。",
    )
...
def setup_client(config, *, enable_trace=False):
    ...
    try:
        ...
        if not getattr(client, "connected", False):
            print(f"[*] 正在連接到 {config.OPC_SERVER} @ {config.OPC_HOST}...")
...
def browse_sample_tags(client: OpcDaClient, pattern: str, limit: int, recursive: bool) -> Optional[List[str]]:
    ...
    try:
        print(f"[*] 正在瀏覽標籤（pattern={pattern}, recursive={recursive}）...")
...
def _read_single_tag_with_timeout(client: OpcDaClient, tag: str, timeout_ms: int = 5000) -> Tuple:
    """
    讀取單一標籤，並有明確的逾時控制
    """
    import threading
    ...
    thread.start()
    thread.join(timeout=timeout_ms / 1000.0 + 2.0)  # 多給 2 秒緩衝

    if thread.is_alive():
        print(f"       [✗] 讀取逾時（超過 {timeout_ms}ms）")
        return None
...
def _read_tags_individually(client: OpcDaClient, tags: Sequence[str], timeout_ms: int = 5000) -> bool:
    print("    → 正在逐一讀取以找出問題標籤...")
    any_success = False
    for tag in tags:
        try:
            result = _read_single_tag_with_timeout(client, tag, timeout_ms)
            if result is None:
                print(f"       [✗] {tag}: 讀取逾時或無回應")
                continue
...
def _read_and_print(client: OpcDaClient, tags: Sequence[str], label: str, use_async: bool = False, timeout_ms: int = 10000) -> bool:
    tags = list(tags)
    if not tags:
    ...
    sync_mode = not use_async
    mode_str = "SyncRead" if sync_mode else "AsyncRead"
    print(
        f"    → 使用 {mode_str}（source=device, timeout={timeout_ms}ms）",
        flush=True,
    )

    try:
        print(f"    → 開始讀取 {len(tags)} 個標籤...")
        results = client.read(
            tags,
            sync=sync_mode,
            source="device",
        ...
        print(f"    → 讀取完成，收到 {len(results) if results else 0} 筆結果")
    except Exception as exc:
        print(f"[✗] {label} 讀取失敗：{exc}")
        import traceback
        traceback.print_exc()
        print("\n    → 改用逐一讀取模式...")
        return _read_tags_individually(client, tags, timeout_ms)
...
def read_sample_tags(client: OpcDaClient, sample_tag: str, group_tags: Sequence[str], use_async: bool = False) -> bool:
    ...
    single_ok = _read_and_print(client, [sample_tag], "單一標籤", use_async)

    group_ok = True
    if group_tags:
        group_ok = _read_and_print(client, group_tags, f"群組標籤（前 {len(group_tags)} 筆）", use_async)
    else:
...
def run_diagnostics(config: OpenOpcConfig, all_tags: List[str], args) -> Tuple[bool, Optional[OpcDaClient]]:
    ...
    read_ok = read_sample_tags(client, sample_tag, group_tags, args.use_async)
    diag_ok &= read_ok

    return diag_ok, client


def read_values_in_batches(client, all_tags, batch_size=5, timeout_ms=10000, use_async=False):
    """
    分批讀取 OPC 資料點的值（改進版）
    """
    all_data = []
    ...
    print(f"\n[*] 開始讀取 {len(all_tags)} 個資料點（批次大小: {batch_size}）[{fmt_date}]")
    print("-" * 100)
    ...
    for batch_idx, batch_tags in enumerate(batches, 1):
        print(f"\n[批次 {batch_idx}/{total_batches}] 讀取 {len(batch_tags)} 個標籤...")
        ...
        try:
            ...
            if not all_values:
                print(f"[✗] 批次 {batch_idx} 沒有收到任何資料")
                continue

            print(f"[✓] 批次 {batch_idx} 成功讀取 {len(all_values)} 個資料點")
            ...
            print(f"[✗] 批次 {batch_idx} 讀取失敗: {e}")
            import traceback
            traceback.print_exc()
            continue

    print("-" * 100)
    print(f"[✓] 總計成功讀取 {len(all_data)} 個資料點")
    return all_data
...
def main_loop(args):
    ...
    try:
        if not args.skip_diagnostics:
            ...
            if not diag_ok:
                print("\n[!] 診斷未完全通過，但仍可以嘗試繼續執行擷取。")
                print("[!] 若持續失敗，請檢查 DCOM 權限設定。")
            if args.diag_only:
                print("[*] 診斷步驟已完成（diag-only），程式結束。")
                return

        if client is None:
            _print_step("[*] 建立主要客戶端連接")
            client = setup_client(config, enable_trace=args.debug_opc)
            if not client:
                print("[✗] 無法連接客戶端，程式終止")
                return
        ...
        print(f"[*] 使用批次大小: {args.read_batch_size}")
        print(f"[*] 使用模式: {'非同步' if args.use_async else '同步'}\n")
        ...
        while True:
            ...
            all_data = read_values_in_batches(
                client,
                all_tags,
